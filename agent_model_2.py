			...
			if self.state == 0:
				...
				if not np.any(mask):
					...
					tumor[new_pos] = TumorCell(new_pos, 
						...
						phagocytosis_rate=self.phagocytosis_rate)
...
class NKCell():
	...
	def __init__(self, pos, movement_speed=1, detection_probability=0.2, cytokines_gen=12, vanish_rate=0.1, incoming_rate=0.1):
		#Parameters
		self.ms = movement_speed
		self.dp = detection_probability
		self.cr = cytokines_gen
		self.vr = vanish_rate
		self.ir = incoming_rate
		#Variables
		self.pos = pos
		self.state = 0
		self.moving = False
	...
	def cycle(self, tumor, nkcells, resources, cytokines, nm_nk, nm_tumor):
		if self.state == 0 and not self.moving:
			...
			if np.any(tmask):
				...
				if np.random.uniform(0,1,1)[0] < self.vr:
			...
			if np.any(tmask):
				if tumor[nm_tumor[np.argmax(tmask)]].state != 3:
					...
					if np.random.uniform(0,1,1)[0] < self.ir:
						nbohrs = nm_nk + self.pos
						mask = (nkcells[nbohrs] == None)
						if np.any(mask):
							pos = nbohrs[np.argmax((mask*np.random.uniform(1, 2, mask.size)))]
							nkcells[pos] = NKCell(pos,
								movement_speed=self.ms,
								detection_probability=self.dp,
								cytokines_gen=self.cr,
								vanish_rate=self.vr,
								incoming_rate=self.ir)
	...
	def __init__(self, params):
		...
		for pos in init_pos:
			self.nkcells[-1][pos] = NKCell(pos,
				...
				vanish_rate=params["nkcell_vanish_rate"],
				incoming_rate=params["nkcell_incoming_rate"])
